...
input double TakeProfit = 23 ; // How much the price should go up before we take the profit an walk away.
input double BuyPrice =  6 ;  // What price do we want to buy at
input double StopLoss = 5;


input int MCAD_Fast = 19;
input int MCAD_Slow = 24;
input int MCAD_Signal = 12;
...
void OnTick()
  {
   ...
   if (NewBarTime != Time[0])
   {
      ...
      //Alert("New bar.");
      if (iOsMA(NULL,0,MCAD_Fast,MCAD_Slow,MCAD_Signal,PRICE_CLOSE,0) > iOsMA(NULL,0,MCAD_Fast,MCAD_Slow,MCAD_Signal,PRICE_CLOSE,1))
      {
